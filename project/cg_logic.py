...
class CoreGrowthLogic(CoreGrowthRiskTacticalMixin):
    ...
    def CheckDdCircuitBreaker(self) -> bool:
        ...
        try: self.CgShadowReplayPushCtx("dd_cb","CheckDdCircuitBreaker","MANDATORY_OVERRIDE",emergency=True)
        except Exception: pass
        self.liquidate()   # full portfolio including SH
        try: self.CgShadowReplayPopCtx()
        except Exception: pass
    ...
    def TriggerEmergencyLiquidation(self):
        ...
        if can_park:
            ...
            try: self.CgShadowReplayPushCtx("emergency","TriggerEmergencyLiquidation","MANDATORY_OVERRIDE",emergency=True)
            except Exception: pass
    ...
    def MergeSleeves(self, core_targets: dict, overlay_targets: dict) -> dict:
        ...
        max_symbol_weight = float(getattr(self, "max_symbol_weight", 2.50))
        ...
        max_total_exposure = float(getattr(self, "max_total_exposure", 1.90))
    ...
    def ExecuteTargets(self, targets, reduce_only=False):  # [LSS2]
        _ecl = "MANDATORY_OVERRIDE" if reduce_only else "NORMAL_SHADOW_ELIGIBLE"
        try: self.CgShadowReplayPushCtx("ExecuteTargets","ExecuteTargets",_ecl,reduce_only=bool(reduce_only),shadow_eligible=not reduce_only,targets=targets)
        except Exception: pass
        try:
